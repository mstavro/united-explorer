...
main_tbl <- main_tbl |> mutate(amenity_table = map(registration, \(x) generate_addt_info_table(main_tbl, x))) |>
      select(delivered, aircraft_image, aircraft_model, aircraft_seatmap, amenity_table) |>
      gt() |>
      ...
      tab_style(
        list(
          ...
          cell_text(color = "#FFFFFF", weight = 400, stretch = "semi-condensed", size = "large")
        ),
      ...
      opt_interactive(use_search = TRUE, page_size_values = c(5,10,25), page_size_default = 5, use_filters = TRUE)
