...
generate_map_tables <- function(df, specified_callsign, output_html = TRUE){
  ...
  united_logo <- web_image("https://www.united.com/2500e4e62233fbfe8ac6.unitedLogoNew.svg", height = "1em")
  ...
  origin_value_box <- value_box(value = tbl$origin_airports_iata, title = html(glue("{tbl$origin_airports_name}<br>{tbl$origin_airports_countryiso2}")), showcase = fa("plane-departure", height = "4.5em"), theme = value_box_theme
  ("bg-gradient-blue-purple"))} else{
    origin_value_box <- value_box(value = glue("{tbl$velocity} m/s"), title = "Aircraft Velocity", theme = value_box_theme("bg-gradient-blue-purple"), showcase = fa("angles-right", height = "3em"))
  }
  dest_value_box <- if(tbl$destination_plausible == 1){value_box(value = tbl$destination_airports_iata, title = html(glue("{tbl$destination_airports_name}<br>{tbl$destination_airports_countryiso2}")), showcase = fa("plane-arrival", height = "4.5em"), theme = value_box_theme("bg-gradient-blue-purple"))} else {
    dest_value_box <- value_box(value = glue("{tbl$baro_altitude} meters"), title = "Altitude", theme = value_box_theme("bg-gradient-blue-purple"), showcase = if(tbl$vertical_rate > 0){fa("angle-up", height = "3em")} else if(tbl$vertical_rate == 0){fa("minus", fill = "#FFFFFF", "3em")} else{fa("angle-down", height = "3em")}
  )
  }

  if(tbl$origin_plausible == 0 | tbl$destination_plausible == 0){
    reliability <- glue("{fa('triangle-exclamation', fill = '#990000')} <b>Route data for {callsign} is unreliable. Operational information is displayed instead.</b>")
  } else{
    reliability <- glue("{fa('triangle-exclamation', fill = '#FFB343')} <b>Route data for {callsign} is plausible, but may still be outdated/incorrect. Double check using other sources.</b>")
  }
  ...
  get_flag <- function(x, height = "1em") {
    ...
    country_names <- gt:::country_names
        ...
        x_str_non_missing <- vapply(seq_along(x_str_non_missing), 
            FUN.VALUE = character(1L), USE.NAMES = FALSE, FUN = function(x) {
                if (grepl(",", x_str_non_missing[x], fixed = TRUE)) {
                  countries <- toupper(unlist(strsplit(x_str_non_missing[x], 
                    ",\\s*")))
                } else {
                ...
                for (y in seq_along(countries)) {
                  ...
                  flag_svg <- flag_tbl[flag_tbl[[paste0("country_code_", 
                    country_i_len)]] == country_i, ][["country_flag"]]
                  out_y <- gsub("<svg.*?>", paste0("<svg xmlns=\"http://www.w3.org/2000/svg\" ", 
                    "aria-hidden=\"true\" role=\"img\" ", "width=\"512\" height=\"512\" ", 
                    "viewBox=\"0 0 512 512\" ", "style=\"vertical-align:-0.125em;", 
                    "image-rendering:optimizeQuality;", "height:", 
                    height, ";", "width:", height, ";", "\"", 
                    ">"), flag_svg)
                  out <- c(out, out_y)
                }
                paste0("<span style=\"white-space:nowrap;\">", 
                  paste0(out, collapse = ""), "</span>")
            })
  ...
  gt_tbl <- tbl |> mutate(across(everything(), \(x) as.character(x))) |> 
    ...
    if(destination_plausible == 0 & is.na(destination_airports_name)){
      ...
      as.character(html(glue("{destination_airports_iata} - {destination_airports_name} {get_flag(destination_airports_countryiso2)}")))
      }) |>
    ...
    mutate(amenity_ife = case_when(
      ife == "NO" ~ NA,
      str_detect(ife, "No ") ~ NA,
      str_detect(ife, "AVOD|DTV") & str_detect(ife, "PDE") ~ glue('{web_image("https://media.united.com/assets/m/6902cf38052408ed/original/x-global-icons-travel-airport-on-demand.svg")} {web_image("https://media.united.com/assets/m/36ec366e93188fda/original/x-global-icons-travel-airport-personal-device.svg")}'),
      str_detect(ife, "AVOD|DTV") ~ web_image("https://media.united.com/assets/m/6902cf38052408ed/original/x-global-icons-travel-airport-on-demand.svg"),
      str_detect(ife, "Seatback|SEATBACK") ~ web_image("https://media.united.com/assets/m/6902cf38052408ed/original/x-global-icons-travel-airport-on-demand.svg"),
      str_detect(ife, "PDE") ~ web_image("https://media.united.com/assets/m/36ec366e93188fda/original/x-global-icons-travel-airport-personal-device.svg")
    )) |>
    mutate(amenity_wifi = case_when(
      wifi == "NO" ~ NA,
      .default = web_image("https://media.united.com/assets/m/4d2b44a2bcff27d9/original/x-global-icons-travel-airport-wifi.svg")
    )) |>
    mutate(amenity_power = case_when(
      power == "NO" ~ NA,
      power == "F/E+" ~ NA,
      .default = web_image("https://media.united.com/assets/m/3acf299d26de7a3b/original/x-global-icons-travel-airport-in-seat-power.svg")
    )) |>
    mutate(amenity_power = case_when(
      str_detect(power, "USB") ~ glue("{amenity_power} {web_image('https://media.united.com/assets/m/601a3d239d42470/original/USB-icon.svg')}"),
      .default = amenity_power
    )) |>
    mutate(amenities = glue("{amenity_wifi} {amenity_power} {amenity_ife}", .na = "")) |>
    select(registration, aircraft_model, delivered, config, amenities, velocity, true_track, vertical_rate, baro_altitude, squawk, special_purpose, origin_info, destination_info) |> 
    rename(
...
generate_addt_info_table <- function(df, specified_registration){
  ...
  tbl |> 
    select(aircraft_model, config, amenities, ife, wifi, power, J, F, PP, `E+`, Y) |>
